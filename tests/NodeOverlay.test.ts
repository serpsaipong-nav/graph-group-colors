...
describe("NodeOverlay", () => {
  it("skips overlay creation for zero or one color", () => {
    ...
    const overlay = new NodeOverlay(container, () => new FakeGraphics(), { maxColorsPerNode: 6 });
  ...
  it("caps slices based on maxColorsPerNode", () => {
    const container = new FakeContainer();
    const overlay = new NodeOverlay(container, () => new FakeGraphics(), { maxColorsPerNode: 2 });
